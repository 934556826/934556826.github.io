<template>
  <div class="home">
    <Homenav :active="true" :nactive="false" />
    <HomeSearch />

    <van-swipe :autoplay="3000">
      <van-swipe-item v-for="(image, index) in images" :key="index">
        <img style="width:100%; height:144px" v-lazy="image" />
      </van-swipe-item>
    </van-swipe>

    <van-tabbar route>
      <van-tabbar-item replace to="/home">
        <img slot="icon" :src="img[0]" alt />
        书架
      </van-tabbar-item>
      <van-tabbar-item replace to="/home">
        <img slot="icon" :src="img[1]" alt />
        排行
      </van-tabbar-item>
      <van-tabbar-item replace to="/home">
        <img slot="icon" :src="img[2]" alt />
        VIP
      </van-tabbar-item>
      <van-tabbar-item replace to="/home">
        <img slot="icon" :src="img[3]" alt />
        充值
      </van-tabbar-item>
    </van-tabbar>
    <div class="jilu">
      <p>
        最近阅读 |
        <span>暂无阅读记录</span>
      </p>
    </div>
    <div class="female-one">
      <h3>男生热门</h3>
      <ul>
        <Book v-for="(item, index) in manone" :key="index" :item="item" 
        
        />
      </ul>
    </div>
    <div class="female-two">
      <h3>男生完本</h3>
      <ul>
        <Book v-for="(item, index) in mantwo" :key="index" :item="item" 
        
        />
      </ul>
    </div>
    <div class="female-three">
      <h3>男生大神区</h3>
      <ul>
        <Book v-for="(item, index) in manthree" :key="index" :item="item" 
        
        />
      </ul>
    </div>
  </div>
</template>

<script>
// @ is an alias to /src
import Homenav from "@/components/Homenav.vue";
import HomeSearch from "@/components/HomeSearch.vue";
import Book from "@/components/Book.vue";

export default {
  name: "home",
  components: {
    Homenav,
    HomeSearch,
    Book
  },
  data() {
    return {
      images: [
        "https://plf-new.zhuishushenqi.com/management/images/20191227/c4e95cae27c04439a440103c119d52ad.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20191227/b5250a3c8c30427188bfc15e178832a9.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20191218/89499eb80a5b4a16a5191330a84749a5.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20191210/d945ea2a6f514efe996f6c1153333798.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20191218/3c40285aca8d45fbaa7913ae25d712a9.jpg",
        "https://plf-new.zhuishushenqi.com/management/images/20191210/41e6baf0a67b4219807f9acc98a9940a.jpg"
      ],
      img: [
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,",
        "data:image/png;base64,"
      ],
      manone: [],
      mantwo: [],
      manthree: []
    };
  },
  methods: {
    getdata() {
      this.axios
        .get(
          "http://novel.kele8.cn/category-info?gender=male&type=hot&major=奇幻&minor=&start=0&limit=12"
        )
        .then(response => {
          //   window.console.log(response);
          this.manone = response.data.books.slice(0, 4);
          this.mantwo = response.data.books.slice(4, 8);
          this.manthree = response.data.books.slice(8, 12);
        });
    }
  },
  created() {
    this.getdata();
  }
};
</script>
<style lang="less">


h3 {
  border-left: 3px solid red;
  padding: 0 5px;
  font-size: 14px;
  height: 18px;
  line-height: 18px;
  
}

.jilu {
  padding: 20px;
  & > p {
    color: #999;
    font-size: 10px;
  }
}
.van-tabbar {
  margin-top: 20px;
}

.van-tabbar-item {
  & > .van-tabbar-item__icon > img {
    width: 43px;
    height: 43px;
    // display: block;
  }
}
.van-tabbar--fixed {
  position: static !important;
  height: auto !important;
}
</style>


