...
function addFriendListEvent() {
  ...
  for (var i = 0; i < lis.length; i++) {
    lis[i].onclick = function () {
      ...
      //点击关闭事件
      var bbb = document.getElementsByClassName('close-btn')[0];
      bbb.onclick = function () {
        getId('chat-right').style.display = 'none';
        self.className = 'clearfix';
        document.getElementsByClassName('title')[0].textContent = '联系人';
      }
...
window.onload = function () {
  ...
  //回车发送信息
  getId('input-content').onkeypress = function (event) {
    if (event.which == 13) {
      recover();
